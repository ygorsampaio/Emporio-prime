...
export default function CartDrawer() {
  ...
  function handleCheckout() {
    ...
    const msg = `Olá, Empório Prime!%0A%0APedido:%0A${lines}%0A%0A*Total: R$ ${total.toFixed(2).replace('.', ',')}*`
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
              <AnimatePresence initial={false}>
                {items.length === 0 ? (
                  ...
                  items.map((item) => <CartItemRow key={item.id} item={item} onRemove={removeItem} onQty={updateQty} />)
                )}
              </AnimatePresence>
            </div>
            ...
            {items.length > 0 && (
              <div className="px-6 py-5 border-t border-cream-200 space-y-3">
                ...
                <button
                  ...
                  className="w-full py-3.5 bg-forest-500 hover:bg-forest-700 text-white rounded-xl font-semibold transition-colors shadow-sm"
                >
